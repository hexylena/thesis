...
mapping before genome maps could be generated with ease {[}1,2{]}. While
...
application {[}3{]}. Scientists around the world are collecting phages
...
their distribution {[}4,5{]}. Coupling this with a surge in use of phage
...
understanding of fundamental phage biology {[}6{]}.

The great sequencing explosion has resulted in many new viral {[}7{]}
...
{[}8{]}, and new phage-specific annotation pipelines are command-line
based making teamwork less fluid {[}9{]}. While the capability to do the
...
manual inspection for high-quality annotation is needed {[}10{]}. The
...
biologists to perform analyses on biological sequence data {[}11{]}.
...
{[}12,13{]}. The CPT created a Galaxy-Apollo bridge to link the powerful
...
(\protect\hyperlink{pcbi.1008214.g001}{Fig 1}) {[}14{]}. To date, more
...
\centering
\includegraphics{info:doi/10.1371/journal.pcbi.1008214.g001}
...
discussion on its use for phage annotation {[}11{]}. The user interface
...
writing new scripts to suit specialized analysis needs {[}15--17{]}. The
...
SwissProt, NCBI nt and nr, as well as certain custom databases {[}18{]}.
...
chromosome down to the DNA sequence {[}19{]}. Apollo is a web-based
...
resources provided by the Galaxy Training Network {[}20{]}. Tutorials
...
``Shine\_Dalgarno\_sequence'' (a sequence ontology term {[}21,22{]}),
...
frame within the sequence of another {[}23{]}. Spanins can function as a
...
at a Cys residue at the end of a lipobox motif {[}24,25{]}. Their varied
...
to the suite of tools that we have developed to find them {[}26{]}.
...
(ORFs) or coding sequences (CDSs)} {[}16,17{]} finding all possible ORFs
...
{[}23{]} is determined by BLASTp with a 0.001 expectation value cutoff.
2) Potential lipobox motifs expected for outer membrane spanins are
identified using the tools \textbf{LipoP} {[}27{]} and \textbf{Identify
...
which predicts transmembrane regions {[}28{]}. Finally, to reduce noise,
...
consistent with the architecture of known spanin genes {[}23{]}.
...
while phage regularly contain group I introns {[}29--31{]}. In the case
of T4 and T4-like phages, these mobile elements are seen to vary
substantially {[}32--34{]}. The most reliable way to find the true
...
which λ is the best characterized example {[}35--40{]}. When the
...
BLAST {[}41{]}. While the number of sequenced phage genomes present in
the NCBI database has increased rapidly since next-generation sequencing
became widespread {[}42,43{]}, so have sequences of cellular organisms
and eukaryotic viruses {[}44{]}. Sequences outside bacteria and phages
...
that are required by BLAST 2.9 {[}45{]}. Using curated lists of TaxIDs
...
\textless200 kb {[}46{]}, they usually assemble completely in a single
...
circular {[}43{]}. As with other genome browsers, wraparounds are not
...
{[}47{]}. These software assessments can be useful in determining the
...
\centering
\includegraphics{info:doi/10.1371/journal.pcbi.1008214.g002}
...
genes {[}48,49{]}. Their outputs are converted into GFF3 format and
...
{[}50{]}. A third gene caller, \textbf{Get open reading frames (ORFs) or
...
both a start and stop codon {[}16,17{]}. The GetORFs output is further
...
{[}52,53{]}. With a typical \textgreater90\% coding density,
...
{[}53--55{]}, which is why the CPT Galaxy pipeline is designed to allow
...
\centering
\includegraphics{info:doi/10.1371/journal.pcbi.1008214.g003}
...
information from several other services {[}56--59{]}. In the CPT Galaxy
...
using TMHMM 2.0 {[}28{]}. The simple output is a GFF3 file, which, when
...
bigwig format for a plot format display {[}60{]}, and mapped back onto
...
incorporated into the InterProScan software package {[}59,61{]}.
...
\centering
\includegraphics{info:doi/10.1371/journal.pcbi.1008214.g004}
...
generate a dot plot; MIST builds upon Gepard {[}62{]} to generate a
...
{[}65{]}. Another useful workflow set, \emph{X-vis from GFF3/Fasta},
...
{[}66{]}. Thousands of students have performed phage annotation through
...
for functional annotation of phage isolates {[}9{]} One unique aspect in
multiPhATE is an accompanying tool designed specifically to detect phage
genes called PHANOTATE {[}54{]}.
...
RefSeq sequences {[}67,68{]}. DFAST is a downloadable and web-based
prokaryotic genome annotation pipeline that boasts a 10-minute or less
processing time for full bacterial genomes {[}69{]}. The RASTtk pipeline
...
Resource Integration Center) {[}70--72{]}. Their use of subsystems to
perform quick and automatic annotations has also been applied to phage
genome annotation, recently described in manual format {[}73{]}.
...
we have done in several instances {[}74--76{]}. Finally, the system's
...
A list of all tools used in the structural (S), functional (F), and
comparative genomics (C) workflows.

The current working versions of the tools presented here are available
at \url{https://github.com/TAMU-CPT/galaxy-tools}.

(XLSX)

S2 Table

The consensus and derivative possible Shine-Dalgarno sequences that are
the default in ShineFind.

By default, the ShineFind tool searches within 3--24 nucleotides
upstream of a given start codon for the longest, or first, possible
Shine-Dalgarno sites that match to the following, starting with the
\emph{E}. \emph{coli} consensus.
...
Each category may be used separately or together to restrict a BLAST
analysis by TaxID. Phage common names and NCBI accession numbers are
included in the custom canonical phages database, which is also
pre-compiled as a BLAST database. Bolded rows contain the well-studied
...
Helpful CPT Galaxy workflow links.

Current versions of all published workflows can be accessed at
\url{https://cpt.tamu.edu/galaxy-pub/workflows/list_published}.
...
Comparison of phage genome annotations performed using different
methods.

Five phages, the canonical phage T1 and four previously annotated at the
CPT, were assessed with RASTtk and features were compared. The number of
gene features listed is strictly a count, not reflecting the possibility
that completely different genes, or genes with alternate starts, were
called. The hypothetical protein count is meant to be a proxy for
functional annotation. Since RASTtk also uses the nonspecific name
``phage protein'', this term was separately tallied. In the additional
features row are listed genes and other aspects of phage genomes that
are typically only identified by manual inspection, and which only our
pipeline identifies here. tmp = tape measure protein, and terminators
refer to rho-independent terminators predicted by TransTermHP. Compiled
annotation data can be browsed by the public at
\url{https://cpt.tamu.edu/apollo/jbrowse/}.
