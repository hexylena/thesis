...
\begin{figure}[t!]
\centering
\includegraphics[height=10em]{chapters/cancer/htsget.png}
\end{figure}
\vspace{-4cm}
...
We also implemented the clin.iobio tool within the Galaxy framework. Clin.iobio is a workflow for precision genomics. The workflow is able to create dynamic results, which include creating quality control plots, creating a list of genes for the disease of interest, creating an automatic report of pathogenic variants within the list of genes, allowing the custom filtering of pathogenic variants, reporting phenotypes and publications related to the gene of interest, and reviewing the variants. These are major improvements compared to the existing trio analysis tool within Galaxy, GEMINI, which was only able to produce static plots or large lists of filtered variants.

\subsubsection*{Workflow \& Tutorial}
In this study we illustrate an end-to-end workflow for trio analysis for FAIR data. This workflow retrieves and analyses files from large datasets in the EGA and can easily be adapted to any other EGA dataset. We illustrate this by analysing data from the EGAD00001008392 \cite{egad} dataset. This dataset contains 6 trio families with different inheritance patterns of digitally spiked-in variants, where each family is subject to a different disease. Next, we demonstrate the usefulness of clin.iobio by analysing the family trios and comparing the existing trio analysis tool in Galaxy, GEMINI, to the clin.iobio tool. Finally, a comprehensive tutorial is made available at the Galaxy Training materials \cite{GTN} under the topic `Variant Analysis` titled `Trio Analysis using Synthetic Datasets from RD-Connect GPAP)` \cite{tutorial} to teach users how to access data from the EGA and to recreate and run the workflow from scratch.

\subsubsection*{Use Case: Breast Cancer}
Here we report on the output produced by clin.iobio to demonstrate it's added value to the Galaxy platform. To produce these results we used case 5 from the EGA dataset. This case describes a family trio where the mother and daughter are affected by breast cancer. The case describes an autosomal dominant inheritance pattern, which causes a missense single nucleotide polymorphism (SNP) at chromosome 17 position 41,215,920 changing a guanine into a thymine. \cite{egad} 
...
After downloading the data from the EGA the workflow pre-processes the data and produces multiple outputs using clin.iobio.

Firstly, clin.iobio shows a dashboard with quality control figures, see figure \ref{fig:qc_clin}. The figure shows the given family pedigree, the distribution of coverage across different regions, and the variant type distribution. The figure shows that the VCFs of all family members are of good quality. Also, both the coverage distribution and variant distribution of the sampled variants are similar between family members.

% Quality control figure
\begin{figure*}[bt!] %% preferably at bottom or top of column
    \centering
    \includegraphics[width=\linewidth]{chapters/images/htsget/figure2.pdf}
    \caption{Quality control figures created by clin.iobio. Here a) shows a visual representation of the family pedigree, b) shows the distribution of coverage across different regions, and c) shows the variant type distribution.}\label{fig:qc_clin}
\end{figure*}

% Generated list of genes
Secondly, a disease or phenotype of interest can be provided to produce a list of genes of interest. In this case the disease is breast cancer and the associated phenotypes are breast carcinoma and neoplasm of the breast. The automatic selection of important genes related to the disease speeds up the process of finding causative variants. Alternatively, genes can be added manually if there are any of particular interest.

% Pathogenic Variants
Finally, clin.iobio searches, by default, for causative variants in the top 20 of provided list of genes by filtering all the variants in the VCFs using pre-selected, but customizable, parameters. Figure \ref{fig:variant} shows that a spiked-in causative variant was found with sufficient depth and allele counts. In addition, clin.iobio shows a visualization of the coverage around the causative variant, statistics on conservation of the region, and a pathogenicity score of the variant. This information helps the user to determine the legitimacy of the variant.

\begin{figure*}[t!] %% preferably at bottom or top of column
    \centering
    \includegraphics[width=\linewidth]{chapters/images/htsget/figure3.pdf}
    \caption{Overview of clin.iobio results for the spiked-in variant. The figure shows statistics on quality of the variant, phenotype associations, pathogenicity, population frequency, inheritance, and conservation.}\label{fig:variant}
\end{figure*}

Overall, clin.iobio provides an interactive and visual overview of causative variant identification. This is a significant improvement compared to the previous causative variant identification tool GEMINI. Especially with regards to identifying the quality of the causative variant as illustrated by figure \ref{fig:qc_clin} and \ref{fig:variant}.

\subsubsection*{Trio Analysis Comparison}
% Table comparing if the variant was found back
In addition, we further validated the clin.iobio tool by identifying the causative variants in all the families available. A comparison of the results reported by clin.iobio and GEMINI is shown in table \ref{tab:comparison}. It shows the number of variants reported by GEMINI and clin.iobio using the default parameters. Numbers with a star indicate that the gene of interest was not in the top 20 of the automatically generated gene list of clin.iobio. Therefore, the gene was manually added to check if clin.iobio can still find the causative variant. Table \ref{tab:comparison} shows that GEMINI does not report any variants for some cases. When GEMINI does report variants it reports the correct variants. However, it also reports many false positive, since each family has only one or two spiked-in causative variants. In contrast, clin.iobio does report causative variants for each family and only the correct ones. This shows that clin.iobio is not only interpretable but also accurate.

\begin{table}[bt!]
    \caption{Overview of existing Galaxy trio analysis tools and the number of variants they report.}\label{tab:comparison}
    \begin{tabular}{c c c}
        \toprule
        Family & GEMINI & clin.iobio \\
        \midrule
        Case1 & 0 & 1 \\ % 209  46
        Case2 & 77 & 1 \\ % 23  12
        Case3 & 0 & 2 \\ % 6    1
        ...
        \bottomrule
    \end{tabular}
    \caption{\textsuperscript{*} Indicates the gene of interest was not in the top 20 of the gene list in clin.iobio.}
\end{table}
...
Clin.iobio is run with the GRCh37 reference genome and the data type is set to Genome with no bed file. The gene list is created using the disease and phenotypes described in Additional file 1. The default search filters are used to detect the causative variant.

\subsection*{GEMINI}
For each case GEMINI is prompted to remove low impact severity variants and to search for causative variants that match the inheritance pattern in Additional file 1. 

\section*{Availability of source code and requirements}

\subsection*{PyEGA3}
\begin{itemize}
    \item Project name: pyega3
...
\subsection*{clin.iobio}
\begin{itemize}
    \item Project name: clin.iobio
    \item Github repository: \url{}
    \item ToolShed repository: \url{}
    \item Training Manual: \url{https://training.galaxyproject.org/training-material/topics/variant-analysis/tutorials/trio-analysis/tutorial.html}
    \item Operating system(s): Unix (Platform independent with Docker, Singularity)
    \item Other requirements: Galaxy version 22.05 or higher
    \item License: MIT
...
% Test case 5
The data used in this study were generated by a public human WGS experiment in the Illumina Platinum initiative \cite{Eberle}, which was made available by the HapMap project \cite{HapMap}. All data from this project is available at the EGA website \cite{EGA}. In this study only the BAMs and VCFs with the chromosomes containing the spiked-in variants were included. These files are available under `RD-Connect GPAP synthetic data spiked-in variant data` at Zenodo \cite{zenodo}. 
...
AAI: Authentication and Authorisation, BAM: Binary alignment map, B1MG: Beyond 1 Million Genomes, DAC: Data Access Committee, EGA: European Genome-Phenome Archive, FAIR: Findable, Accessible, Interoperable, and Reusable, GA4GH: Global Alliance for Genomics and Health, GTN: Galaxy Training Network, GVCF: Genomic variant calling format, OIDC: OpenID Connect, SNP: Single nucleotide polymorphism, VCF: Variant call format.

\subsection*{Competing Interests}
The authors declare that they have no competing interests.

\subsection*{Funding}
This project has received funding from the EC H2020 project CINECA (grant 825775) \cite{CINECA}. The data used in this project was created with the support of the RD-Connect GPAP \cite{RD_connect_GPAP}, EC H2020 project EJP-RD (grant 825575) \cite{EJP-RD}, EC H2020 project B1MG (grant 951724) \cite{B1MG_grant}, and Generalitat de Catalunya project VEIS (grant 001-P-001647) \cite{VEIS}.

\subsection*{Author's Contributions}
J.O., S.H., H.R., and D.S. contributed to writing the manuscript. J.O., S.H., H.R. contributed to writing the Galaxy workflow tutorial. S.H., H.R., and D.D. contributed to implementing the PyEGA3 and clin.iobio tool in Galaxy. A.P.S. supervised the project. All authors approved the manuscript.

\section*{Acknowledgements}
We would like to thank the Galaxy community and in particular Wolfgang Maier, for reviewing, testing, and validating the workflow tutorial, PyEGA3 and clin.iobio tool.
