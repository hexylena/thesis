%!TEX TS-program = xelatex
%!TEX encoding = UTF-8 Unicode

\documentclass[School=EMC]{Dissertate}
\fancypagestyle{plain}{
\fancyhf{}
\fancyfoot[EL,OR]{\thepage}
}

\usepackage{tikz}

% xelatex -shell-escape -output-driver="xdvipdfmx -z 0" <filename>.tex
% \usepackage[x-4]{pdfx}
\usetikzlibrary{arrows.meta, positioning}
\tikzset{%
    block/.style = {draw=red,rectangle,thick,
        minimum height=2em, text width=5em, align=center},
    line/.style = {draw=black, line width=2pt, 
        -{Triangle[length=5pt, width=5pt]}, shorten >=2pt, shorten <=2pt},
}
\usepackage[sectionbib]{chapterbib}
\usepackage{tabularx}
\usepackage{enumitem}
% \usepackage[none]{hyphenat} % Disables all word breaking
\usepackage{subcaption}
\usepackage{graphicx}
\usepackage{wrapfig}
\usepackage[hyphens]{xurl} % fixes url splitting over lines
\usepackage[overlap, CJK]{ruby}
\usepackage{nth}
\usepackage{metalogo}
\usepackage{cleveref}
\usepackage{paracol}


% Book size: 17 x 24 cm
% Bleed needed by ipskamp: 2 mm
% Margins recommended by IPSkamp: top & inner: 2.5cm, bottom & outer: 2 cm
\usepackage[paperwidth=17.4cm, paperheight=24.4cm, inner=2.6cm, outer=2.1cm,bottom=2.4cm,top=2.2cm]{geometry}


% place page number on outside of page (left for even pages, right for odd)
\renewcommand{\headrulewidth}{0pt}
\fancyhead{}
\fancyhf{}
\input{vars}

%\usepackage[width=6.75in, letterpaper, bottom=3cm, top=3cm]{geometry}
%\newcommand\bookepigraph[4]{
%\vspace{1em}\hfill{}\begin{minipage}{#1}{\begin{spacing}{0.9}
%\small\noindent\textit{#2}\end{spacing}
%\vspace{1em}
%\hfill{}{#3}\\

%\vspace{-1em}\begin{flushright}{#4}\end{flushright}}\vspace{2em}
%\end{minipage}}

\def\changemargin#1#2{\list{}{\rightmargin#2\leftmargin#1}\item[]}
\let\endchangemargin=\endlist

\newcommand\epigraph[3]{
    \vspace{1em}\begin{center}
    \begin{minipage}{#1}{
        \begin{spacing}{1.2}
        \small\noindent\textit{#2}\end{spacing}
        \vspace{1em}
        \hfill{}\small{#3}
    }
    \vspace{2em}
    \end{minipage}
    \end{center}
}



%\newcommand\anonymousepigraph[2]{
%\vspace{1em}\hfill{}\begin{minipage}{#1}{\begin{spacing}{0.9}
%\small\noindent\textit{#2}\end{spacing}}
%\vspace{1em}
%\end{minipage}}
\usetikzlibrary{calc}

% Orange
\definecolor{title1}{HTML}{8c0000}
\definecolor{title2}{HTML}{ff3c00}
\definecolor{title3}{HTML}{b90000}
\definecolor{title4}{HTML}{ff8080}

% Blue
\definecolor{title1}{HTML}{8c0000}
\definecolor{title2}{HTML}{003cff}
\definecolor{title3}{HTML}{0000b9}
\definecolor{title4}{HTML}{8080ff}


% hyphenation
\usepackage[british]{babel}
\babelhyphenation[british]{bio-infor-ma-tician}
\babelhyphenation[british]{bio-infor-ma-ticians}
\babelhyphenation[british]{bio-infor-ma-tics}
\setlength\emergencystretch{3em}

% ensuring certain pages appear on left or right side
\newcommand*\cleartoleftpage{%
  \clearpage
  \ifodd\value{page}\hbox{}\newpage\fi
}

\newcommand*\cleartorightpage{%
  \clearpage
  \ifodd\value{page} \else \hbox{}\newpage \fi
}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Thumb index

\usepackage[contents={},opacity=1,scale=1,color=black]{background}
\usepackage{tikzpagenodes}
\usepackage{totcount}
\usetikzlibrary{calc}

\newif\ifMaterial

\newlength\LabelSize
\setlength\LabelSize{2.5cm}

\AtBeginDocument{%
\regtotcounter{chapter}
\setlength\LabelSize{\dimexpr\textheight/\totvalue{chapter}\relax}
\ifdim\LabelSize>2.5cm\relax
  \global\setlength\LabelSize{2.5cm}
\fi
}

\newcommand\AddLabels{%
\Materialtrue%
\AddEverypageHook{%
\ifMaterial%
\ifodd\value{page} %
  \backgroundsetup{
  angle=270,
  position={current page.east|-current page text area.north east},
  vshift=-15pt,
  hshift=\thechapter*\LabelSize,
  contents={%   ,draw=hexy-red, line width=0.2mm,
  \tikz\node[fill=gray!35,anchor=west,text width=\LabelSize,
    align=center,text height=30pt,text depth=10pt,font=\large\rmfamily] {\thechapter};
  }%
 }
 \else
 \backgroundsetup{
  angle=90,
  position={current page.west|-current page text area.north west},
  vshift=-15pt,
  hshift=-\thechapter*\LabelSize,
  contents={%
  \tikz\node[fill=gray!35,anchor=west,text width=\LabelSize,
    align=center,text height=30pt,text depth=10pt,font=\large\rmfamily] {\thechapter};
  }%
 }
 \fi
 \BgMaterial%
\else\relax\fi}%
}

\newcommand\RemoveLabels{\Materialfalse}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% End Thumb index


\usepackage{listings}
\newcommand\doi[1]{\href{https://doi.org/#1}{doi:#1}}
\definecolor{codegreen}{rgb}{0,0.6,0}
\definecolor{codegray}{rgb}{0.5,0.5,0.5}
\definecolor{codegray20}{rgb}{0.2,0.2,0.2}
\definecolor{codepurple}{rgb}{0.58,0,0.82}
\definecolor{backcolour}{rgb}{1,1,1}

\lstdefinestyle{mystyle}{
    backgroundcolor=\color{backcolour},
    commentstyle=\color{codegray20},
    keywordstyle=\color{hexy-red},
    numberstyle=\tiny\color{codegray20},
    stringstyle=\color{hexy-red},
    basicstyle=\ttfamily\footnotesize,
    breakatwhitespace=false,
    breaklines=true,
    captionpos=b,
    keepspaces=true,
    numbers=left,
    numbersep=5pt,
    showspaces=false,
    showstringspaces=false,
    showtabs=false,
    tabsize=2,
    frame=bt
}

\newcommand\interstitial[1]{%
\cleardoublepage%
{\thispagestyle{empty}%
\begin{tikzpicture}[remember picture,overlay]
\node[inner sep=0pt] at (current page.center){%
    \includegraphics[width=\paperwidth,height=\paperheight]{chapters/#1-front.png}%
};
\end{tikzpicture}
}
\clearpage
\begin{tikzpicture}[remember picture,overlay]
\node[inner sep=0pt] at (current page.center){%
    \includegraphics[width=\paperwidth,height=\paperheight]{chapters/#1-back.png}%
};
\end{tikzpicture}
}

\newcommand\intentionallyBlank{%
{\thispagestyle{empty}%
\begin{tikzpicture}[remember picture,overlay]
\node[inner sep=0pt] at (current page.center){%
    \includegraphics[width=\paperwidth,height=\paperheight]{chapters/blank2.png}%
};
\end{tikzpicture}
}\clearpage
}

\newcommand\includePdf[1]{%
{\thispagestyle{empty}%
\begin{tikzpicture}[remember picture,overlay]
\node[inner sep=0pt] at (current page.center){%
    \includegraphics[width=\paperwidth,height=\paperheight]{#1}%
};
\end{tikzpicture}
}\clearpage
}


% \input{frontmatter/glossary.tex}


\lstset{style=mystyle}
%% Custom

\begin{document}
\pagestyle{empty}

% the front matter
%\newpage \thispagestyle{empty} This page was intentionally left blank (inside cover).\newpage

% \include{frontmatter/cover}

\setcounter{page}{1}
\input{frontmatter/personalize}

\maketitle
%\maketitledutch
\pagestyle{fancy}

% \fancyhead[C]{\iffloatpage{}{\texttt{\gitRev}}}
% \fancyhead[C]{\iffloatpage{}{\texttt{\gitRevSimple}}}
% \fancyfoot[ER,OL]{\includegraphics[width=1.5em]{barcode/a\thepage.png}}
% \fancyhead[EL]{\leftmark}
% \fancyhead[OR]{\rightmark}

\fancyhead[EL,OR]{\iffloatpage{}{\includegraphics[width=0.5em]{aesthetics/cis/\thepage.png}}}
\fancyfoot[EL,OR]{\iffloatpage{}{\thepage}}



%\copyrightpage
%\abstractpage
\dedicationpage


%\include{frontmatter/stellingen}

\cleartorightpage
% \intentionallyBlank\clearpage
\include{frontmatter/foreword}
%\include{frontmatter/stellingen}


\setlength\cftparskip{-5pt}
\setlength\cftbeforechapskip{0pt}

\setcounter{tocdepth}{1}
\cleartorightpage
\tableofcontents
%\phantomsection
%\addcontentsline{toc}{chapter}{Table of Contents}

% \listoffigures

\cleartorightpage

\AddLabels % start using thumb index

% start chapter and page numbering at 0 because (computer) science
\setcounter{chapter}{-1}
\include{chapters/introduction}

% \interstitial{platform}
\include{chapters/icell}
	\include{chapters/phages}
	\include{chapters/circos}

\include{chapters/iclinic}
	\include{chapters/ecpc}
	\include{chapters/htsget}

\include{chapters/iworld}
	\include{chapters/gtn}
	\include{chapters/tiaas}


\include{chapters/discussion}
\RemoveLabels %stop thumb index

\begin{appendices}
  % \include{endmatter/glossary}

  \include{endmatter/ss} % DONE

  \include{endmatter/publications} % DONE
  \include{endmatter/cv}           % DONE
  \include{endmatter/portfolio}    % DONE
  \include{endmatter/acknowledgments}    % DONE
\end{appendices}


% \begin{appendices}
%   \include{endmatter/code}
% \end{appendices}

% UNCOMMENT FOR FINAL
% \cleartorightpage

% \acknowledgments

% \intentionallyBlank
% \intentionallyBlank

\cleartoleftpage
\include{endmatter/colophon}
\clearpage


%\bibliography{references}
%\addcontentsline{toc}{chapter}{References}
%\bibliographystyle{apalike2}
\bibliographystyle{hexy.bst}

% \include{frontmatter/cover-back}

\end{document}
%!TEX TS-program = xelatex

